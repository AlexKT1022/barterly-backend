...
export const createUser = async (username, password, profileImageUrl = null, location = null) => {
  ...
  try {
    ...
    if (err?.code === 'P2002') { const e = new Error('Username is already taken'); e.status = 409; throw e; }
    throw err;
  }
};
...
export const updateMe = async (id, { username, profile_image_url, location }) => {
  try {
    ...
    if (e?.code === 'P2002') { e.status = 409; e.message = 'Username already taken'; }
    throw e;
  }
};
...
export const changeMyPassword = async (id, old_password, new_password) => {
  ...
  if (!ok) { const e = new Error('Old password is incorrect'); e.status = 400; throw e; }
...
export const listMyPosts = async (id, { status, limit = 20, offset = 0 } = {}) => {
...
export const getMyActivity = async (id, { limit = 20, offset = 0 } = {}) => {
  ...
  const [myNewPosts, responsesOnMyPosts, myResponses, myTrades] = await Promise.all([
    ...
    prisma.trade.findMany({
      where: { OR: [{ post: { authorId: userId } }, { response: { authorId: userId } }] },
  ...
  const items = [
    ...myNewPosts.map(p => ({ type: 'post_created', at: p.createdAt, post_id: p.id, title: p.title })),
    ...responsesOnMyPosts.map(r => ({ type: 'response_on_my_post', at: r.createdAt, post_title: r.post.title, from: r.author.username, response_id: r.id })),
    ...myResponses.map(r => ({ type: 'my_response', at: r.createdAt, post_title: r.post.title, response_id: r.id })),
    ...myTrades.map(t => ({ type: 'trade', at: t.agreedAt, status: t.status, post_title: t.post.title, trade_id: t.id })),
  ]
