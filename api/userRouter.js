...
import { getUserById, createUser, getUserByCredentials } from '#db/queries/userQueries';
...
router.post(
  ...
  async (req, res, next) => {
    try {
      ...
      const user = await createUser(username, email, password, profile_image_url, location);
